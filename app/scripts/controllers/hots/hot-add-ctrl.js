teamunApp.controller('HotAddCtrl', ['$scope', '$state', '$stateParams', 'toaster', '$timeout', 'Upload', '$qupload', '$log', 'QiniuService', 'HotService', 'DEFAULT_DOMAIN', function($scope, $state, $stateParams, toaster, $timeout, Upload, $qupload, $log, QiniuService, HotService, DEFAULT_DOMAIN) {

  $scope.saveHot = function(hot) {
    hot.poster = $scope.poster;
    HotService.saveHot.save($.param(hot), function(data) {
      $state.go('admin.hot-list');
    }, function(error) {
