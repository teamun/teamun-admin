teamunApp.controller('ActivityEditCtrl', ['$scope', '$location', '$state', '$stateParams', 'toaster', '$timeout', 'Upload', '$qupload', '$log', 'ActivityService', 'EventService', 'TeamService', 'UserService', 'CityService', 'OrganizerService', 'QiniuService', 'DEFAULT_DOMAIN', function($scope, $location, $state, $stateParams, toaster, $timeout, Upload, $qupload, $log, ActivityService, EventService, TeamService, UserService, CityService, OrganizerService, QiniuService, DEFAULT_DOMAIN) {
  ...
  $scope.updateActivity = function(activity) {
    ...
    ActivityService.updateActivity.update({activity_id: $stateParams.activity_id}, $.param(activity), function(result) {
      if(result.ret === 1) {
        $location.path('/admin/activity-detail/' + $stateParams.activity_id);
      } else {
